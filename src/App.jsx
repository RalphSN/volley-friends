...
export default function App() {
  ...
  const [profile, setProfile] = useState(null); // { nickname, shareCode }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (rosterSnap.exists()) {
        ...
        showToast("資料匯入成功！已覆蓋現有圖鑑。", "success");
        setImportCode("");
        setImportConfirm(false);
      } else {
      ...
      <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-slate-50">
        {customFields.map((field) => {
          ...
          if (field.type === "choice" && Array.isArray(rawVal)) {
            ...
            const platformLabels = { ig: "IG", fb: "FB", line: "LINE" };
            const contactFilled =
              rawVal && rawVal.id && rawVal.id.trim() !== "";
            return (
              <div
                key={field.id}
                className={`col-span-2 bg-slate-50/80 p-3 rounded-2xl text-sm border border-slate-100 flex justify-between items-center transition-colors hover:bg-slate-100 ${
                  !contactFilled ? "opacity-60" : ""
                }`}
              >
                <span className="text-slate-500 font-medium flex items-center gap-1.5">
                  <Phone size={14} /> {field.name}
                </span>
                <span
                  className={`font-medium ${
                    !contactFilled ? "text-slate-400" : ""
                  }`}
                >
                  {!contactFilled ? (
                    "未填寫"
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-emerald-600 underline hover:text-emerald-500 inline-flex items-center gap-1 bg-emerald-50 px-2 py-1 rounded-md"
                    >
                      <ExternalLink size={14} />{" "}
                      {platformLabels[rawVal.platform]}: {rawVal.id}
                    </a>
                  )}
                </span>
              </div>
          ...
          if (field.type === "rating") {
            return (
              <div
                key={field.id}
                className={`bg-slate-50/80 p-2.5 rounded-2xl flex justify-between items-center text-sm border border-slate-100 transition-colors hover:bg-slate-100 ${
                  isEmpty ? "opacity-60" : ""
                }`}
              >
          ...
          if (field.type === "yesno" || field.type === "choice") {
            return (
              ...
              >
                ...
                <span
                  className={`font-bold px-3 py-1 rounded-lg text-xs shadow-sm transition-colors ${
                    isEmpty
          ...
          return (
            <div
              key={field.id}
              className={`col-span-2 bg-slate-50/80 p-3 rounded-2xl text-sm border border-slate-100 transition-colors hover:bg-slate-100 ${
                isEmpty ? "opacity-60" : ""
              }`}
            >
  ...
  // ====== 登入後的主畫面 ======
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans selection:bg-emerald-100 text-slate-800 relative">
      {/* 全域 Toast 顯示 */}
      <div
        className={`fixed top-6 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-full shadow-[0_10px_40px_rgba(0,0,0,0.1)] font-bold text-sm flex items-center gap-2 transition-all duration-500 ${toast.show ? "translate-y-0 opacity-100" : "-translate-y-10 opacity-0 pointer-events-none"} ${toast.type === "success" ? "bg-emerald-600 text-white" : "bg-red-500 text-white"}`}
      >
            ...
            <div>
              <div className="flex gap-3 mb-8">
                ...
                <button
                  ...
                  className={`cursor-pointer whitespace-nowrap px-5 rounded-2xl flex items-center justify-center transition-all duration-300 shadow-sm border active:scale-95 ${
                    showFilters ||
              ...
              {/* ====== 展開的進階篩選器 ====== */}
              {showFilters && (
                <div className="bg-white p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100 mb-8 animate-slide-up origin-top">
                  <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4 border-b border-slate-100 pb-5">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <select
                        ...
                        className="bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-xl px-3 py-2 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 cursor-pointer transition-all"
                      >
                        ...
                        {customFields
                          ...
                          .map((f) => (
                            <option key={`sort-${f.id}`} value={f.id}>
                              依 {f.name} 高低
                            </option>
                          ))}
                      </select>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {POSITIONS.map((pos) => (
                        <button
                          ...
                          className={`cursor-pointer px-4 py-2 rounded-xl font-medium text-sm transition-all duration-300 border active:scale-90 ${
                            filterPositions.includes(pos)
                          ...
                          >
                            ...
                            <div className="flex gap-2">
                              ...
                              <input
                                ...
                                className="flex-1 bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 outline-none focus:border-emerald-400"
                              />
                            </div>
                          </div>
                        );
                  ...
                  <div className="mt-6 flex justify-end">
                    <button
                      ...
                      className="cursor-pointer whitespace-nowrap text-sm font-semibold text-slate-400 hover:text-slate-600 transition-colors px-4 py-2 rounded-xl hover:bg-slate-100 active:scale-95"
                    >
                      清除所有條件
                    </button>
                  </div>
                      ...
                      {customFields.map((field, index) => {
                        ...
                        return (
                          <div
                            key={field.id}
                            className={
                              !isLast ? "border-b border-slate-100 pb-6" : ""
                            }
                          >
                            {field.type === "rating" ? (
                              <RatingInput
                              ...
                              <div>
                                <label className="font-bold text-slate-700 flex items-center gap-2 mb-3">
                                  ...
                                  {field.name} {field.isMulti ? "(可複選)" : ""}
                                </label>
                                <div className="flex flex-wrap gap-2.5">
                                  {(field.options || []).map((opt) => {
                                    const currentVal =
                                      formData.attributes[field.id];
                              ...
                              <div>
                                ...
                                <div className="flex gap-2">
                                  <select
                                    value={
                                      formData.attributes[field.id]?.platform ||
                                      "ig"
                                    }
                                    onChange={(e) => {
                                  ...
                                  <input
                                    type="text"
                                    value={
                                      formData.attributes[field.id]?.id || ""
                                    }
                                    onChange={(e) => {
                                      const current = formData.attributes[
                                        field.id
                ...
                <div className="bg-slate-800/60 p-5 rounded-2xl border border-slate-700/50 mb-6 group">
                  ...
                  <div className="flex items-center justify-between bg-slate-950 p-4 rounded-xl border border-slate-700 group-hover:border-emerald-500/50 transition-colors">
                    ...
                    <button
                      ...
                      className="cursor-pointer whitespace-nowrap px-4 py-2 rounded-lg font-bold text-sm transition-all flex items-center gap-1 active:scale-90 bg-slate-700 hover:bg-slate-600 text-white"
                    >
                      <Copy size={16} /> 複製
                    </button>
                  </div>
                  <p className="text-xs text-slate-400 mt-3 font-medium leading-relaxed">
                    將這組代碼分享給好友，他們就能將你辛苦建立的資料整包匯入他們的帳號！
                  </p>
                </div>
                ...
                <div className="bg-slate-800/60 p-5 rounded-2xl border border-slate-700/50">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      className="flex-1 bg-slate-950 border border-slate-700 rounded-xl px-5 py-3 text-white outline-none focus:border-emerald-500 font-mono text-lg uppercase tracking-widest transition-colors disabled:opacity-50"
                      maxLength={6}
                    ...
                    {!importConfirm ? (
                      <button
                        ...
                        className="cursor-pointer whitespace-nowrap bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 disabled:text-slate-500 text-white px-6 py-3 rounded-xl font-bold transition-all active:scale-95 disabled:active:scale-100 flex items-center justify-center gap-2"
                      >
                        準備匯入
                      </button>
                    ) : (
                  ...
                  {importConfirm && (
                    <p className="text-red-400 text-xs mt-3 font-bold flex items-center gap-1 animate-slide-up">
        ...
        <div className="flex flex-1 md:flex-none">
          {[
            ...
            <button
              ...
              className={`cursor-pointer whitespace-nowrap flex-1 md:flex-none md:w-28 py-3.5 md:py-4 flex flex-col items-center gap-1.5 transition-all duration-300 relative group ${
                activeTab === tab.id
