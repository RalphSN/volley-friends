...
const PLATFORM_LABELS = { ig: "ig", fb: "fb", line: "Line", phone: "phone" };
...
export default function App() {
  ...
  const [profileForm, setProfileForm] = useState({ nickname: "", newPassword: "" });
  ...
  useEffect(() => {
    if (!user) return;
    const docRef = doc(db, 'artifacts', appId, 'users', user.uid, 'roster', 'data');
    setIsSyncing(true);
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      if (isLoginMode) {
        ...
        await setDoc(doc(db, 'artifacts', appId, 'users', newUid, 'profile', 'info'), {
          nickname: authForm.nickname.trim(),
          email: authForm.email,
          shareCode: newShareCode
        });
        await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'shareCodes', newShareCode), { uid: newUid });
        await setDoc(doc(db, 'artifacts', appId, 'users', newUid, 'roster', 'data'), {
          friends: [],
      ...
      else if (err.message.includes("auth/user-not-found")) msg = "找不到此帳號";
      else if (err.message.includes("auth/wrong-password") || err.message.includes("auth/invalid-credential")) msg = "密碼錯誤";
      else if (err.message.includes("auth/email-already-in-use")) msg = "此信箱已註冊過";
      else if (err.message.includes("auth/weak-password")) msg = "密碼長度需至少 6 個字元";
      showToast(msg, "error");
    } finally {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      if (profileForm.nickname.trim() && profileForm.nickname !== profile.nickname) {
         await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'info'), {
           ...profile,
           nickname: profileForm.nickname.trim()
         });
         setProfile(prev => ({ ...prev, nickname: profileForm.nickname.trim() }));
      }
      if (profileForm.newPassword) {
         if (profileForm.newPassword.length < 6) throw new Error("密碼長度需至少 6 個字元");
  ...
  const handleImportByCode = async () => {
    ...
    setIsImporting(true);
    try {
      const codeRef = doc(db, 'artifacts', appId, 'public', 'data', 'shareCodes', importCode.toUpperCase());
      ...
      const rosterRef = doc(db, 'artifacts', appId, 'users', targetUid, 'roster', 'data');
  ...
  const processFileImport = async (file) => {
    ...
    try {
      ...
      let importedFriends = [];
      let importedFields = customFields; // 預設保留現有欄位
      
      // 容錯判斷：可能是單純陣列，或是包含 customFields 的物件
      ...
      // 修補球友資料確保結構完整 (如果屬性缺少在渲染時自動留空，多餘會被忽略)
      importedFriends = importedFriends.map(f => ({
        ...f,
        id: f.id || Date.now().toString() + Math.random().toString(36).substr(2, 5),
        name: f.name || "未命名球友",
  ...
  // 計算是否啟用篩選
  const activeFiltersCount = filterPositions.length + Object.keys(filterValues).length + (sortBy !== "skill" && sortBy !== "" ? 1 : 0);
  ...
  const handleSaveFriend = async (e) => {
    ...
    let newFriends;
    if (formData.id) {
      newFriends = friends.map((f) => f.id === formData.id ? { ...formData } : f);
    } else {
  ...
  const handleSaveField = async (e) => {
    ...
    if (editingFieldId) {
      const updatedOthers = otherFields.map((f) => f.id === editingFieldId ? newField : f);
  ...
  const moveField = async (index, direction) => {
    ...
    if (direction === "up" && index > 0) {
      [newEditable[index - 1], newEditable[index]] = [newEditable[index], newEditable[index - 1]];
  ...
  const getInitialAttributeValue = (field) => {
    if (field.type === "rating") return null; // 評分預設為空
  ...
  const openEditForm = (friend) => {
    ...
    setSelectedFriend(null); // 關閉 Modal
    setActiveTab("form");
  };

  const togglePosition = (pos, currentList, onChange) => {
    if (currentList.includes(pos)) onChange(currentList.filter((p) => p !== pos));
  ...
  const getContactLink = (contact) => {
    ...
    switch (contact.platform) {
      case "ig": return `https://www.instagram.com/${id}`;
      case "fb": return `https://www.facebook.com/${id}`;
      case "line": return `https://line.me/ti/p/~${id}`;
  ...
  const filteredFriends = useMemo(() => {
    let result = friends.filter((friend) => {
      if (filterName && !friend.name.toLowerCase().includes(filterName.toLowerCase())) return false;
      if (filterPositions.length > 0) {
        const hasMatchingPosition = friend.positions.some((pos) => filterPositions.includes(pos));
      ...
      for (const field of customFields) {
        ...
        if (fVal !== undefined && fVal !== null) {
          ...
          if ((field.type === "choice" || field.type === "yesno") && Array.isArray(fVal) && fVal.length > 0) {
            if (!friendVal) return false;
            const friendValArr = Array.isArray(friendVal) ? friendVal : [friendVal];
    ...
    return result;
  }, [friends, filterName, filterPositions, filterValues, sortBy, customFields]);
  ...
  const RatingInput = ({ label, value, onChange }) => (
    <div className="mb-5 animate-slide-up">
      <div className="flex justify-between items-center mb-3">
        ...
        <span className={`text-sm font-bold px-3 py-1 rounded-full shadow-sm ${value ? 'text-emerald-600 bg-emerald-50' : 'text-slate-400 bg-slate-100'}`}>
          {value ? `Lv. ${value}` : "未評分"}
        </span>
      </div>
  ...
  const FriendCard = ({ friend, posKey }) => {
    // 尋找要顯示的精簡欄位
    const genderField = customFields.find(f => f.id === 'gender' || f.name.includes('性別'));
    const heightField = customFields.find(f => f.id === 'height' || f.name.includes('身高'));
    ...
    return (
      <div 
        ...
        className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm hover:shadow-[0_10px_40px_rgb(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-300 group cursor-pointer"
      >
              ...
              {friend.positions.map((p) => (
                <span
                  ...
                  className={`text-xs px-2.5 py-1 rounded-md font-bold transition-colors shadow-sm ${
                    p === posKey ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-500"
                  }`}
                >
                  {p}
                </span>
          ...
          <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
            {confirmDeleteId === friend.id ? (
              <div className="flex items-center gap-2 bg-red-50/80 p-1.5 rounded-xl border border-red-100 animate-fade-in">
                <button onClick={() => setConfirmDeleteId(null)} className="cursor-pointer text-slate-600 text-xs px-3 py-1.5 bg-white rounded-lg border border-slate-200 font-medium hover:bg-slate-50 active:scale-95 transition-transform">取消</button>
                <button onClick={() => handleDeleteFriend(friend.id)} className="cursor-pointer text-white text-xs px-3 py-1.5 bg-red-500 rounded-lg font-bold shadow-sm shadow-red-500/20 hover:bg-red-600 active:scale-95 transition-transform">刪除</button>
              </div>
            ) : (
              <div className="opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-300 flex gap-1 translate-x-2 md:translate-x-0">
                <button onClick={() => openEditForm(friend)} className="cursor-pointer p-2.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-full transition-all hover:rotate-12 active:scale-90"><Edit2 size={18} /></button>
                <button onClick={() => setConfirmDeleteId(friend.id)} className="cursor-pointer p-2.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-all hover:-rotate-12 active:scale-90"><Trash2 size={18} /></button>
              </div>
            )}
          </div>
        </div>
        ...
        {/* 精簡資訊區塊 */}
        <div className="flex flex-wrap gap-x-4 gap-y-2 mt-4 pt-4 border-t border-slate-50 text-sm">
          {genderVal && <div className="text-slate-600 flex items-center gap-1"><User size={14} className="text-slate-400"/>{genderVal}</div>}
          {heightVal && <div className="text-slate-600 flex items-center gap-1"><Hash size={14} className="text-slate-400"/>{heightVal} cm</div>}
          {contactVal && contactVal.id && (
             <div className="text-slate-600 flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
               {contactVal.platform === 'phone' ? <Smartphone size={14} className="text-emerald-500"/> : <Phone size={14} className="text-emerald-500"/>}
               <a href={getContactLink(contactVal)} target="_blank" rel="noreferrer" className="text-emerald-600 hover:underline">
                 {PLATFORM_LABELS[contactVal.platform]}: {contactVal.id}
               </a>
             </div>
          )}
        </div>
      ...
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-4 relative">
        ...
        <div className="text-center mb-8 animate-slide-up">
          <div className="bg-white p-4 rounded-full shadow-sm inline-block mb-4 border border-slate-100">
             <span className="text-5xl block transform hover:rotate-12 transition-transform duration-300">🏐</span>
          </div>
          <h1 className="text-3xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-emerald-600 to-teal-500">
            排球圖鑑 Volley Friends
          </h1>
          <p className="text-slate-500 font-medium mt-2">紀錄並管理你的排球好友圈</p>
        </div>

        <div className="bg-white w-full max-w-md p-8 rounded-3xl shadow-[0_15px_50px_rgba(0,0,0,0.05)] border border-slate-100 animate-slide-up" style={{ animationDelay: '0.1s' }}>
          <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center justify-center gap-2">
            {isLoginMode ? <><LogIn size={24} className="text-emerald-500"/> 登入帳號</> : <><UserPlus size={24} className="text-emerald-500"/> 註冊新帳號</>}
          </h2>

          <form onSubmit={handleAuthSubmit} className="space-y-4">
            {!isLoginMode && (
              <div>
                <label className="block font-bold text-slate-700 mb-1.5 text-sm">暱稱</label>
            ...
            <div>
              <label className="block font-bold text-slate-700 mb-1.5 text-sm">電子信箱</label>
              <div className="relative">
            ...
            <div>
              <label className="block font-bold text-slate-700 mb-1.5 text-sm">密碼</label>
              <div className="relative">
            ...
            >
              {isAuthenticating ? <RefreshCw size={20} className="animate-spin" /> : isLoginMode ? "登入" : "立即註冊"}
            </button>
          </form>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans selection:bg-emerald-100 text-slate-800 relative pb-32">
      ...
      {/* ====== 球員詳細資訊 Lightbox ====== */}
      {selectedFriend && (
        <div className="fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-sm flex justify-center items-center p-4 animate-fade-in" onClick={() => setSelectedFriend(null)}>
          <div 
          ...
          >
             <div className="sticky top-0 bg-white/90 backdrop-blur-md px-6 py-4 border-b border-slate-100 flex justify-between items-center z-10">
                <div>
                   <h2 className="text-2xl font-extrabold text-slate-800">{selectedFriend.name}</h2>
                   <div className="flex gap-1.5 flex-wrap mt-2">
                      {selectedFriend.positions.map((p) => (
                        <span key={p} className="bg-emerald-500 text-white text-xs px-2.5 py-1 rounded-md font-bold shadow-sm">{p}</span>
                      ))}
                    </div>
                </div>
                <button onClick={() => setSelectedFriend(null)} className="p-2 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-full transition-colors active:scale-95 cursor-pointer">
                  <X size={20} />
                </button>
             </div>
             ...
             <div className="p-6 space-y-4">
                {customFields.map((field) => {
                  ...
                  const isEmpty = rawVal === undefined || rawVal === null || rawVal === "" || (Array.isArray(rawVal) && rawVal.length === 0);
                  let displayVal = isEmpty ? "-" : rawVal;

                  if (field.type === "choice" && Array.isArray(rawVal)) displayVal = rawVal.join(", ");
                  else if (field.type === "contact") {
                    const contactFilled = rawVal && rawVal.id && rawVal.id.trim() !== "";
                    return (
                      <div key={field.id} className={`bg-slate-50/80 p-3 rounded-2xl text-sm border border-slate-100 flex justify-between items-center transition-colors ${!contactFilled ? "opacity-60" : ""}`}>
                        <span className="text-slate-500 font-medium flex items-center gap-1.5"><Phone size={14} /> {field.name}</span>
                        <span className={`font-medium ${!contactFilled ? "text-slate-400" : ""}`}>
                          {!contactFilled ? "未填寫" : (
                            <a href={getContactLink(rawVal)} target="_blank" rel="noreferrer" className="text-emerald-600 underline hover:text-emerald-500 inline-flex items-center gap-1 bg-emerald-50 px-2 py-1 rounded-md">
                              <ExternalLink size={14} /> {PLATFORM_LABELS[rawVal.platform]}: {rawVal.id}
                            </a>
                          )}
                        </span>
                      </div>
                  ...
                  if (field.type === "rating") {
                    return (
                      <div key={field.id} className={`bg-slate-50/80 p-3 rounded-2xl flex justify-between items-center text-sm border border-slate-100 transition-colors ${isEmpty ? "opacity-60" : ""}`}>
                        <span className="text-slate-500 flex items-center gap-1.5 font-medium"><Star size={14} className={isEmpty ? "text-slate-300" : "text-amber-400"} /> {field.name}</span>
                        <span className={`font-bold ${isEmpty ? "text-slate-400" : "text-slate-700"}`}>{isEmpty ? "未評分" : `Lv.${displayVal}`}</span>
                      </div>
                    );
                  }
                  ...
                  if (field.type === "yesno" || field.type === "choice") {
                    return (
                      <div key={field.id} className={`bg-slate-50/80 p-3 rounded-2xl text-sm border border-slate-100 flex justify-between items-center transition-colors ${isEmpty ? "opacity-60" : ""}`}>
                        <span className="text-slate-500 font-medium flex items-center gap-1.5">{field.type === "choice" && field.isMulti ? <CheckSquare size={14} /> : <HelpCircle size={14} />} {field.name}</span>
                        <span className={`font-bold px-3 py-1 rounded-lg text-xs shadow-sm transition-colors ${isEmpty ? "bg-slate-200/50 text-slate-400" : "text-emerald-700 bg-emerald-100/60"}`}>{isEmpty ? "未填寫" : displayVal}</span>
                      </div>
                    );
                  }

                  return (
                    <div key={field.id} className={`bg-slate-50/80 p-3 rounded-2xl text-sm border border-slate-100 transition-colors ${isEmpty ? "opacity-60" : ""}`}>
                      <span className="text-slate-500 flex items-center gap-1.5 text-xs block mb-1 font-medium">{field.type === "number" ? <Hash size={12} /> : <AlignLeft size={12} />} {field.name}</span>
                      <span className={`font-medium ${isEmpty ? "text-slate-300" : "text-slate-700"}`}>{isEmpty ? "未填寫" : displayVal}</span>
                    </div>
                  );
                })}
             </div>
             ...
             <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3 mt-auto">
                <button onClick={() => openEditForm(selectedFriend)} className="flex-1 cursor-pointer bg-white border border-slate-200 text-slate-600 hover:text-emerald-600 hover:border-emerald-300 px-4 py-3 rounded-xl font-bold transition-all active:scale-95 flex justify-center items-center gap-2">
                   <Edit2 size={18}/> 編輯資料
                </button>
                <button onClick={() => handleDeleteFriend(selectedFriend.id)} className="cursor-pointer bg-white border border-slate-200 text-red-500 hover:bg-red-50 hover:border-red-200 px-4 py-3 rounded-xl font-bold transition-all active:scale-95 flex justify-center items-center">
                   <Trash2 size={18}/>
                </button>
             </div>
          </div>
      ...
      {/* ====== 篩選器 Lightbox Modal ====== */}
      {isFilterModalOpen && (
        <div className="fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-sm flex justify-center items-end md:items-center p-4 sm:p-0 animate-fade-in" onClick={() => setIsFilterModalOpen(false)}>
           <div 
           ...
           >
             <div className="sticky top-0 bg-white/90 backdrop-blur-md px-6 py-4 border-b border-slate-100 flex justify-between items-center z-10">
                ...
                <button onClick={() => setIsFilterModalOpen(false)} className="p-2 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-full transition-colors active:scale-95 cursor-pointer">
                  <X size={20} />
                </button>
             </div>
             ...
             <div className="p-6">
                <div className="flex flex-col gap-4 border-b border-slate-100 pb-5 mb-5">
                  <div className="flex items-center gap-3">
                    ...
                    <select
                      ...
                      className="flex-1 bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-xl px-3 py-3 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 cursor-pointer transition-all"
                    >
                      <option value="">預設 (建立順序)</option>
                      {customFields.filter((f) => f.type === "rating" || f.type === "number").map((f) => (
                        <option key={`sort-${f.id}`} value={f.id}>依 {f.name} 高低</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                <div className="mb-6">
                  <label className="block font-bold text-slate-700 mb-3 text-sm tracking-wide">包含位置</label>
                  <div className="flex flex-wrap gap-2">
                    {POSITIONS.map((pos) => (
                      <button
                        key={pos}
                        onClick={() => togglePosition(pos, filterPositions, setFilterPositions)}
                        className={`cursor-pointer px-4 py-2.5 rounded-xl font-medium text-sm transition-all duration-300 border active:scale-90 ${
                          filterPositions.includes(pos) ? "bg-emerald-500 text-white border-emerald-500 shadow-md shadow-emerald-500/20 -translate-y-0.5" : "bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100 hover:border-slate-300"
                        }`}
                      >
                        {pos}
                      </button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {customFields.map((field) => {
                    if (field.type === "rating") {
                      return (
                        <div key={`filter-${field.id}`} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 transition-colors">
                          <label className="flex justify-between text-slate-600 mb-2 font-medium text-sm">
                            <span className="flex items-center gap-1.5"><Star size={14}/> 最低 {field.name}</span>
                            <span className="text-emerald-600 font-bold bg-emerald-100/50 px-2 rounded-md shadow-sm">Lv. {filterValues[field.id] || 1}</span>
                          </label>
                          <input
                            type="range"
                            min="1"
                            ...
                            onChange={(e) => setFilterValues({ ...filterValues, [field.id]: Number(e.target.value) })}
                    ...
                    if (field.type === "choice" || field.type === "yesno") {
                      const options = field.type === "yesno" ? ["是", "否", "不確定"] : field.options || [];
                      ...
                      return (
                        <div key={`filter-${field.id}`} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 transition-colors">
                          <label className="flex items-center gap-1.5 text-slate-600 mb-3 font-medium text-sm">
                            {field.type === 'choice' && field.isMulti ? <CheckSquare size={14}/> : <HelpCircle size={14}/>} {field.name} (可複選)
                          </label>
                          <div className="flex flex-wrap gap-2">
                            {options.map((opt) => (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (arr.includes(opt)) setFilterValues({ ...filterValues, [field.id]: arr.filter((x) => x !== opt) });
                                  else setFilterValues({ ...filterValues, [field.id]: [...arr, opt] });
                                }}
                                className={`cursor-pointer px-3 py-1.5 rounded-lg text-xs font-bold transition-all active:scale-95 border ${
                                  activeOptions.includes(opt) ? "bg-emerald-500 text-white border-emerald-500 shadow-sm" : "bg-white text-slate-500 border-slate-200 hover:bg-slate-100"
                                }`}
                              >
                                {opt}
                              </button>
                    ...
                    if (field.type === "number") {
                      const numFilter = filterValues[field.id] || { op: ">=", value: "" };
                      return (
                        <div key={`filter-${field.id}`} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 transition-colors">
                          ...
                          <div className="flex gap-2">
                            <select
                              value={numFilter.op}
                              onChange={(e) => setFilterValues({ ...filterValues, [field.id]: { ...numFilter, op: e.target.value } })}
                              className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 outline-none focus:border-emerald-400"
                            >
                              <option value=">=">以上 (&ge;)</option>
                              <option value="<=">以下 (&le;)</option>
                            </select>
                            <input
                              type="number"
                              placeholder="輸入數值"
                              value={numFilter.value}
                              onChange={(e) => setFilterValues({ ...filterValues, [field.id]: { ...numFilter, value: e.target.value } })}
                              className="flex-1 w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 outline-none focus:border-emerald-400"
                            />
                          </div>
                        </div>
                      );
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-slate-200/50 pt-safe transition-all">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          >
            <span className="text-emerald-500 transform hover:rotate-12 transition-transform duration-300">🏐</span> 排球圖鑑
          </h1>
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-full bg-slate-100 text-slate-500 border border-slate-200 shadow-sm cursor-default">
              {isSyncing ? <RefreshCw size={14} className="animate-spin text-emerald-500" /> : <Cloud size={14} className="text-emerald-500" />}
              雲端同步中
            </div>
            <button
            ...
            <div>
              <div className="flex flex-col sm:flex-row gap-3 mb-8">
                ...
                <div className="flex gap-2">
                   ...
                   >
                     <Filter size={18} className={activeFiltersCount > 0 ? "fill-emerald-400 text-emerald-100" : "text-slate-400"} /> 
                     篩選與排序 {activeFiltersCount > 0 && <span className="bg-white text-emerald-600 px-1.5 py-0.5 rounded-md text-[10px] ml-1">{activeFiltersCount}</span>}
                   </button>
                   
                   {/* 外部清除篩選按鈕 (若有套用篩選條件) */}
                   {activeFiltersCount > 0 && (
                     <button
                       onClick={() => { setFilterPositions([]); setFilterValues({}); setSortBy("skill"); }}
              ...
              <div className="space-y-12">
                {filteredFriends.length === 0 ? (
                  ...
                  POSITIONS.map((pos) => {
                    const playersInPos = filteredFriends.filter((f) => f.positions.includes(pos));
                    if (filterPositions.length > 0 && !filterPositions.includes(pos)) return null;
                        ...
                        {playersInPos.length === 0 ? (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
                            {playersInPos.map((friend) => (
                              <FriendCard key={`${pos}-${friend.id}`} friend={friend} posKey={pos} />
                            ))}
                          </div>
                        )}
                      </div>
            ...
            <div className="max-w-2xl mx-auto">
              <div className="flex justify-between items-center mb-8">
                ...
                {formData.id && (
                  <button
                    onClick={() => openAddForm()}
                    className="cursor-pointer whitespace-nowrap text-sm text-emerald-600 bg-emerald-50 hover:bg-emerald-100 px-4 py-2 rounded-full font-bold transition-all active:scale-95 hover:shadow-sm"
                  >
                    切換為新增
                  </button>
                )}
                    ...
                    <div className="flex flex-wrap gap-2.5">
                      {POSITIONS.map((pos) => (
                        <button
                          key={pos}
                          type="button"
                          onClick={() => togglePosition(pos, formData.positions, (newPos) => setFormData({ ...formData, positions: newPos }))}
                  ...
                  {customFields.length === 0 ? (
                    ...
                    <div className="space-y-6">
                      {customFields.map((field, index) => {
                        ...
                        return (
                          <div key={field.id} className={!isLast ? "border-b border-slate-100 pb-6" : ""}>
                            {field.type === "rating" ? (
                              <RatingInput
                                label={field.name}
                                value={formData.attributes[field.id]}
                                onChange={(val) => setFormData({ ...formData, attributes: { ...formData.attributes, [field.id]: val } })}
                              />
                            ) : field.type === "yesno" ? (
                              <div>
                                <label className="font-bold text-slate-700 flex items-center gap-2 mb-3">
                                  <HelpCircle size={18} className="text-emerald-500" /> {field.name}
                                </label>
                                <div className="flex gap-3">
                                  {["是", "否", "不確定"].map((opt) => (
                                    <button
                                      key={opt}
                                      type="button"
                                      onClick={() => setFormData({ ...formData, attributes: { ...formData.attributes, [field.id]: opt } })}
                              ...
                              <div>
                                <label className="font-bold text-slate-700 flex items-center gap-2 mb-3">
                                  {field.isMulti ? <CheckSquare size={18} className="text-emerald-500"/> : <List size={18} className="text-emerald-500" />} {field.name} {field.isMulti ? "(可複選)" : ""}
                                </label>
                                <div className="flex flex-wrap gap-2.5">
                                  {(field.options || []).map((opt) => {
                                    const currentVal = formData.attributes[field.id];
                                    const isSelected = field.isMulti ? Array.isArray(currentVal) && currentVal.includes(opt) : currentVal === opt;
                                    const handleClick = () => {
                                      let newVal;
                                      if (field.isMulti) {
                                        const arr = Array.isArray(currentVal) ? currentVal : [];
                                        newVal = arr.includes(opt) ? arr.filter((x) => x !== opt) : [...arr, opt];
                                      } else {
                                        newVal = opt;
                                      }
                                      setFormData({ ...formData, attributes: { ...formData.attributes, [field.id]: newVal } });
                                    };
                                    return (
                                      <button
                                        key={opt}
                                        ...
                                        className={`cursor-pointer whitespace-nowrap px-4 py-2.5 rounded-xl font-bold text-sm transition-all border active:scale-95 ${
                                          isSelected ? "bg-emerald-500 text-white border-emerald-500 shadow-md shadow-emerald-500/20 -translate-y-0.5" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50 hover:border-slate-300"
                                        }`}
                                      >
                                        {opt}
                                      </button>
                              ...
                              <div>
                                <label className="font-bold text-slate-700 flex items-center gap-2 mb-3">
                                  <Phone size={18} className="text-emerald-500" /> {field.name}
                                </label>
                                <div className="flex gap-2">
                                  <select
                                    value={formData.attributes[field.id]?.platform || "ig"}
                                    onChange={(e) => {
                                      const current = formData.attributes[field.id] || { platform: "ig", id: "" };
                                      setFormData({ ...formData, attributes: { ...formData.attributes, [field.id]: { ...current, platform: e.target.value } } });
                                    }}
                                    className="w-1/3 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-4 text-slate-700 outline-none focus:border-emerald-400 font-medium cursor-pointer"
                                  >
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      const current = formData.attributes[field.id] || { platform: "ig", id: "" };
                                      setFormData({ ...formData, attributes: { ...formData.attributes, [field.id]: { ...current, id: e.target.value } } });
                                    }}
                                    placeholder={`輸入帳號或 ID...`}
                                    className="w-2/3 px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-400 transition-all text-slate-800 hover:border-emerald-300"
                                  />
                                </div>
                              </div>
                            ) : (
                              <div>
                                <label className="font-bold text-slate-700 flex items-center gap-2 mb-3">
                                  {field.type === "number" ? <Hash size={18} className="text-slate-400" /> : <AlignLeft size={18} className="text-slate-400" />} {field.name}
                                </label>
                                <input
                                  type={field.type === "number" ? "number" : "text"}
                                  value={formData.attributes[field.id] || ""}
                                  onChange={(e) => setFormData({ ...formData, attributes: { ...formData.attributes, [field.id]: e.target.value } })}
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <form
                ...
                className={`p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border mb-10 transition-all ${
                  editingFieldId ? "bg-emerald-50/50 border-emerald-300 ring-4 ring-emerald-50" : "bg-white border-slate-100 hover:shadow-md"
                }`}
              >
                <div className="flex justify-between items-center mb-5">
                  <h3 className="font-bold text-emerald-600 flex items-center gap-2 text-lg">
                    <div className="p-1.5 bg-emerald-100 rounded-lg shadow-sm">
                      {editingFieldId ? <Edit2 size={18} /> : <Plus size={18} />}
                    </div>{" "}
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    {[
                      ...
                      >
                        <type.icon size={18} className={newFieldType === type.id ? "text-emerald-100" : ""} />
                        {type.label}
                      </button>
                    ))}
                  </div>
                  ...
                  {newFieldType === "choice" && (
                    <div className="space-y-4 animate-slide-up">
                      ...
                      <label className="flex items-center gap-3 cursor-pointer bg-white border border-slate-200 p-4 rounded-2xl hover:bg-slate-50 transition-colors">
                        <div className="relative">
                          <input type="checkbox" className="sr-only" checked={isMultiChoice} onChange={(e) => setIsMultiChoice(e.target.checked)} />
                          <div className={`w-11 h-6 rounded-full transition-colors ${isMultiChoice ? "bg-emerald-500" : "bg-slate-200"}`}></div>
                          <div className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform shadow-sm ${isMultiChoice ? "translate-x-5" : "translate-x-0"}`}></div>
                        </div>
                        <div className="font-bold text-slate-700">
                          允許多選
                          <div className="text-xs text-slate-400 font-normal mt-0.5">開啟後，填寫資料時可同時勾選多個選項</div>
                        </div>
                      </label>
                    </div>
                  )}
              ...
              <div>
                <h3 className="font-bold text-slate-800 mb-4 text-xl">目前的欄位順序</h3>
                  ...
                  {editableFields.map((field, index) => (
                    <div
                      ...
                      className={`flex justify-between items-center bg-white p-4 rounded-2xl border shadow-sm transition-all duration-300 hover:shadow-md ${
                        draggedIdx === index ? "opacity-40 border-emerald-500 scale-[0.98]" : dragOverIdx === index ? "border-emerald-500 border-dashed bg-emerald-50 scale-[1.02]" : "border-slate-200 hover:border-emerald-300"
                      }`}
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div
                          className={`p-3 rounded-xl shadow-sm ${
                            field.type === "rating" ? "bg-amber-100 text-amber-600" : field.type === "text" ? "bg-blue-100 text-blue-600" : field.type === "yesno" ? "bg-emerald-100 text-emerald-600" : field.type === "number" ? "bg-rose-100 text-rose-600" : field.type === "contact" ? "bg-indigo-100 text-indigo-600" : "bg-purple-100 text-purple-600"
                          }`}
                        >
                          {field.type === "rating" ? <Star size={20} /> : field.type === "text" ? <AlignLeft size={20} /> : field.type === "yesno" ? <HelpCircle size={20} /> : field.type === "number" ? <Hash size={20} /> : field.type === "contact" ? <Phone size={20} /> : field.isMulti ? <CheckSquare size={20} /> : <List size={20} />}
                        </div>
                        <div>
                          <div className="font-bold text-slate-800 text-lg">{field.name}</div>
                          <div className="text-xs font-medium text-slate-400 mt-0.5">
                            {field.type === "rating" ? "1-5 評分" : field.type === "text" ? "自由文字" : field.type === "number" ? "數字輸入" : field.type === "contact" ? "聯絡方式" : field.type === "yesno" ? "是非題" : `選擇題 (${field.options?.join(", ")}) ${field.isMulti ? "[可複選]" : ""}`}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-bold text-slate-600 text-lg flex items-center gap-2">
                            {noteField.name} <span className="text-[10px] bg-slate-200 px-2 py-0.5 rounded-full">已鎖定</span>
                          </div>
                          <div className="text-xs font-medium text-slate-400 mt-0.5">系統預設固定於最下方</div>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <div className="bg-gradient-to-br from-indigo-800 to-slate-900 p-6 md:p-8 rounded-3xl shadow-[0_12px_40px_rgb(0,0,0,0.15)] border border-indigo-700 text-white mb-8 hover:shadow-[0_15px_50px_rgb(0,0,0,0.2)] transition-shadow">
                <h3 className="font-bold text-xl mb-6 flex items-center gap-2">
                  <Cloud size={24} className="text-indigo-400 animate-pulse" /> 雲端代碼分享
                </h3>

                <div className="bg-white/10 backdrop-blur-sm p-5 rounded-2xl border border-white/10 mb-6">
                  <h4 className="font-medium text-indigo-300 mb-3 text-sm">你的專屬分享代碼</h4>
                ...
                <div className="bg-white/10 backdrop-blur-sm p-5 rounded-2xl border border-white/10">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      className="flex-1 bg-black/30 border border-white/10 rounded-xl px-5 py-3 text-white outline-none focus:border-indigo-400 font-mono text-lg uppercase tracking-widest transition-colors disabled:opacity-50"
                      maxLength={6}
                    ...
                    {!importConfirm ? (
                      <button
                        ...
                        className="cursor-pointer whitespace-nowrap bg-indigo-500 hover:bg-indigo-400 disabled:bg-slate-700 disabled:text-slate-500 text-white px-6 py-3 rounded-xl font-bold transition-all active:scale-95 disabled:active:scale-100 flex items-center justify-center gap-2"
                      >
                        準備匯入
                      </button>
                    ) : (
                      <div className="flex flex-col sm:flex-row gap-2 animate-fade-in w-full sm:w-auto">
                        <button
                          onClick={handleImportByCode}
                          disabled={isImporting}
                          className="cursor-pointer whitespace-nowrap bg-red-500 hover:bg-red-600 disabled:bg-red-800 text-white px-4 py-3 rounded-xl font-bold transition-all active:scale-95 shadow-lg shadow-red-500/20 text-sm flex items-center justify-center gap-1 w-full sm:w-auto"
                        >
                          {isImporting ? <RefreshCw size={16} className="animate-spin" /> : <AlertCircle size={16}/>} 
                          {isImporting ? "匯入中..." : "確認覆蓋"}
                        </button>
                        <button
                          onClick={() => setImportConfirm(false)}
                          disabled={isImporting}
                          className="cursor-pointer whitespace-nowrap bg-slate-700 hover:bg-slate-600 text-white px-4 py-3 rounded-xl font-bold transition-all active:scale-95 text-sm w-full sm:w-auto"
                        >
                          取消
                        </button>
                      </div>
              ...
              <div className="bg-white p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                <h3 className="font-bold text-xl text-slate-800 mb-6 flex items-center gap-2">
                  <FileJson size={24} className="text-emerald-500" /> 本地檔案備份 (JSON)
                </h3>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex flex-col justify-between">
                    <div>
                      <h4 className="font-bold text-slate-700 mb-2 flex items-center gap-1.5"><DownloadCloud size={16}/> 匯出 JSON 備份</h4>
                  ...
                  <div 
                    className={`border-2 border-dashed rounded-2xl p-5 flex flex-col items-center justify-center text-center transition-all relative ${
                      isDraggingFile ? "border-emerald-500 bg-emerald-50" : "border-slate-200 bg-slate-50 hover:bg-slate-100 hover:border-emerald-300"
                    }`}
                  ...
                  >
                    <input 
                      ...
                      ref={fileInputRef}
                      onChange={(e) => {
                        if (e.target.files[0]) processFileImport(e.target.files[0]);
                        e.target.value = ''; // Reset
                      }}
                    />
                    <UploadCloud size={28} className={isDraggingFile ? "text-emerald-500 mb-2" : "text-slate-400 mb-2"} />
                    <h4 className="font-bold text-slate-700 mb-1 text-sm">匯入 JSON 檔案</h4>
                    <p className="text-[11px] text-slate-500">
                      拖曳檔案至此或點擊選擇<br/>(AI 生成的 JSON 也適用！)
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <div className="bg-white p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 mb-8">
                <div className="flex items-center gap-4 mb-8 pb-6 border-b border-slate-100">
                   ...
                   <div>
                      <h3 className="font-bold text-xl text-slate-800">{profile?.nickname || "使用者"}</h3>
                ...
                <form onSubmit={handleUpdateProfile} className="space-y-6">
                  <div>
                    <label className="block font-bold text-slate-700 mb-2">修改暱稱</label>
                    <div className="relative">
                      <User size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                      <input
                        type="text"
                  ...
                  <div>
                    <label className="block font-bold text-slate-700 mb-2">修改密碼 <span className="text-xs text-slate-400 font-normal ml-2">(若不修改請留空)</span></label>
                    <div className="relative">
                      <Lock size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                      <input
                        type="password"
                  ...
                  >
                    {isUpdatingProfile ? <RefreshCw size={20} className="animate-spin" /> : "儲存修改"}
                  </button>
                </form>
              </div>
      ...
      >
        ...
        <div className="flex flex-1 md:flex-none">
          {[
            ...
            { id: "form", icon: formData.id && activeTab === "form" ? Edit2 : UserPlus, label: formData.id && activeTab === "form" ? "編輯" : "新增" },
            ...
            >
              <div
                className={`transition-all duration-300 transform group-hover:scale-110 group-active:scale-95 ${
                  activeTab === tab.id ? "scale-110 -translate-y-0.5" : "scale-100"
                }`}
              >
